...
public:
        inline SampleOutputWrapper()
        {
			...
			// Inicjalizacja socketu komunikacyjnego.
			context = zmq_ctx_new();
			socket = zmq_socket (context, ZMQ_PUB);
			zmqStatus = zmq_bind (socket, "tcp://*:5556");
        ...
        virtual ~SampleOutputWrapper()
        {
            if (pclFile.is_open())
            {
                ...
                // Zamknięcie socketu.
                zmqStatus = zmq_close (socket);
				zmqStatus = zmq_ctx_destroy (context);
            }
            ...
            if (final)
            {
                for (FrameHessian* f : frames)
                {
                    if (f->shell->poseValid)
                    {
                        ...
                        // Używanie tylko zmarginalizowanych punktów.
                        ...
                        for (auto const* p : points)
                        {
                            ...
                            */
                            ...
                            message = std::to_string(worldPoint[0]) + " " + std::to_string(worldPoint[1]) + " " + std::to_string(worldPoint[2]);
							zmqStatus = zmq_send (socket, message.c_str(), strlen(message.c_str()), 0);
                            ...
                            if (isSavePCL && pclFile.is_open())
                            {
                                ...
                                /*
                                message = std::to_string(worldPoint[0]) + " " + std::to_string(worldPoint[1]) + " " + std::to_string(worldPoint[2]);
								zmqStatus = zmq_msg_init_size (&msg, 32);
								msgData = zmq_msg_data (&msg);
								msgData = &message;
								zmqStatus = zmq_msg_send (&msg, socket, 0);
								*/
                                numPCL++;
                                isWritePCL = false;
                            }
